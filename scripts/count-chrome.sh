...
# "хром полного каркаса с вкладками ≤ ~1500 строк") и пробит ли порог.
#
...
echo
echo "=== ИТОГ ==="
printf 'Хром итого: %d строк. Порог фальсификатора: ~%d строк.\n' "$total_chrome" "$THRESHOLD"

if [ "$total_chrome" -gt "$THRESHOLD" ]; then
  echo "ВЕРДИКТ: ПОРОГ ПРОБИТ ($total_chrome > $THRESHOLD) — по M9-WORKORDER.md это стоп" \
       "и повод для ре-спайка ImGui с замером, решение не принимать молча."
  exit 1
else
  echo "ВЕРДИКТ: порог не пробит ($total_chrome <= $THRESHOLD)."
  exit 0
fi
