...
public static class ReplayCommands
{
    ...
    private const string RecordUsage =
        ...
        + "  --input is a comma-separated list of tick:spec entries on two independent\n"
        + "  tracks, told apart by the spec's first character:\n"
        + "    tick:buttons     what player 0 holds from that tick until the next button\n"
        + "                     entry. Buttons are L R U D O X S (Start); an empty list\n"
        + "                     releases everything. Turns in most carts are edge-triggered\n"
        + "                     (Btnp), so a tap is two entries: \"60:D,61:\".\n"
        + "    tick:mX.Y[LRM][wN]  the pointer from that tick until the next pointer entry:\n"
        + "                     position in screen pixels, held mouse buttons L R M, wheel\n"
        + "                     w<steps> per tick (signed). A click is two entries\n"
        + "                     (\"60:m80.45L,61:m80.45\"), and so is one wheel notch\n"
        + "                     (\"60:m80.45w1,61:m80.45\") -- w repeats every tick.\n"
        + "  Example: --input \"60:D,61:,90:m80.45L,91:m80.45\"\n"
        + "  Every recording is a .qrpr of format version 0, pointer or no pointer: one\n"
        + "  living version and one layout (ADR-041).\n"
