...
public sealed class QuarpGame : Game
{
    ...
    protected override void Update(GameTime gameTime)
    {
        ...
        switch (_modes.Mode)
        {
            ...
            case ShellMode.MapEditor:
                UpdateMapEditor(commands, mouse, gameTime.ElapsedGameTime.TotalSeconds);
                break;
        }
    ...
    /// <summary>
    /// One frame of the map editor (M9 stage 3). Same law as its sibling above: every live
    /// action has a key path and a click path, and both funnel into the same owner so neither
    /// can drift — <see cref="MapEditorPaint"/> for the paint gesture,
    ...
    /// exist precisely because this method cannot be constructed without a
    /// <c>GraphicsDevice</c>: what is written only here can only be tested by a copy of itself
    /// (the wave-2k lesson), so what is written here is the dispatch and nothing else.
    ///
    ...
    /// </summary>
    private void UpdateMapEditor(in ShellCommands commands, in EditorMouse mouse, double elapsedSeconds)
    {
        MapEditorSession map = _modes.MapEditor!;
        MapEditorView view = _modes.MapView!;
        // The same layout the renderer will draw this frame — geometry has one owner.
        ...
        if (view.ExitPromptShown)
        {
            ...
            if (commands.Quit)
            {
                _modes.HandleEscape();                  // Esc lowers the prompt: "stay"
            }
            else if (commands.MenuConfirm)
            {
                _modes.SaveMapAndClose();
            }
            else if (commands.MenuEditor)
            {
                _modes.DiscardMapAndClose();
            }
            else if (mouse.LeftPressed && layout.TryPromptVerb(mouse.X, mouse.Y, out EditorPromptVerb verb))
            {
                switch (verb)
        ...
        if (commands.ToStart)
        {
            _modes.ToggleEditorTab();       // Home: back to the sprites of the same cart
            return;
        }
        if (commands.Quit)
        {
            _modes.HandleEscape();          // clean → the sheet or the library; dirty → the prompt
            return;
        }
        ...
        if (mouse.LeftPressed)
        {
            if (layout.TryButton(mouse.X, mouse.Y, out EditorButton pressed))
            {
                if (!EditorIcons.IsStub(pressed) && HandleMapButton(map, pressed))
                {
    ...
    private bool HandleEditorButton(SpriteEditorSession editor, EditorButton button)
    {
        if (EditorIcons.TabTarget(button) is ShellMode tab)
        {
            ...
            _modes.SwitchEditorTab(tab);
            return _modes.Mode != ShellMode.Editor;
        }
    ...
    /// <summary>
    /// The map screen's twin of <see cref="HandleEditorButton"/>: tabs first (travel is the
    /// mode machine's verb), then <see cref="EditorIcons.ClickMapButton"/>, the headless
    /// routing table the map's button-contract test clicks every placed button through.
    /// Returns true when the click left this mode, so the caller stops touching a session that
    /// may no longer be on screen.
    /// </summary>
    private bool HandleMapButton(MapEditorSession map, EditorButton button)
    {
        if (EditorIcons.TabTarget(button) is ShellMode tab)
        {
            _modes.SwitchEditorTab(tab);
            return _modes.Mode != ShellMode.MapEditor;
        }
